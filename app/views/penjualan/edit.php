...
<div class="bg-white rounded-lg shadow-md p-6">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">
        <i class="fas fa-edit text-yellow-600 mr-2"></i>Edit Penjualan
    </h2>
    ...
    <form action="/penjualan/update/<?= $penjualan['id_penjualan'] ?>" method="POST" id="formPenjualan" onsubmit="return validateForm()">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div class="lg:col-span-2 bg-gray-50 border border-gray-200 rounded-lg p-4">
                ...
                <div id="barang_list" class="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-96 overflow-y-auto">
                    <!-- Barang cards di-generate via JavaScript -->
                </div>
            </div>
        ...
        <div id="payment_section" class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="number" id="uang_diberikan" name="uang_diberikan" min="0" step="0.01" value="<?= $penjualan['uang_diberikan'] ?>"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500"
        ...
        <div class="mb-6">
            ...
            <div id="hutang_section" style="display:<?= $hutangData ? 'block' : 'none' ?>;" class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        ...
        <div class="flex gap-3">
            ...
            <button type="button" onclick="printNota()" class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg transition flex items-center gap-2">
                <i class="fas fa-print"></i>Print Nota
            </button>
            <a href="/penjualan" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg transition flex items-center gap-2">
...
console.log('DEBUG: Edit penjualan, barang count:', allBarang.length, 'detail count:', existingDetails.length);
...
function addItemFromDetail(detail) {
    ...
    const container = document.getElementById('selected_container');
    const noItemsMsg = document.getElementById('no_items_msg');
    const harga = detail.harga_satuan || barangMatch.harga_jual || 0;
    const stok = barangMatch.stok || '';
    const satuan = detail.satuan || barangMatch.satuan || '';

    let itemHtml = '<div style="border:1px solid #90caf9;background:#fff;border-radius:6px;padding:10px;" data-item-index="' + itemIndex + '" data-stok="' + stok + '">';
    itemHtml += '<div style="font-weight:bold;font-size:13px;margin-bottom:6px;">' + (detail.nama_barang || barangMatch.nama_barang || 'Item') + ' <span style="font-size:10px;background:#e3f2fd;color:#1976d2;padding:2px 6px;border-radius:2px;font-family:monospace;">' + (detail.kode_barang || barangMatch.kode_barang || '-') + '</span></div>';
    itemHtml += '<div id="warning_' + itemIndex + '" style="display:none;background:#fff3cd;border:1px solid #ffc107;border-radius:3px;padding:6px;margin-bottom:6px;font-size:12px;color:#856404;"><i class="fas fa-exclamation-triangle"></i> Jumlah melebihi stok tersedia (' + stok + ' ' + satuan + ')</div>';
    itemHtml += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:12px;"><div>';
    itemHtml += '<label style="display:block;color:#666;margin-bottom:4px;">Jumlah Barang</label>';
    itemHtml += '<input type="number" name="items[' + itemIndex + '][jumlah]" value="' + (detail.jumlah || 1) + '" min="1" style="width:100%;padding:4px;border:1px solid #ddd;border-radius:3px;font-size:12px;" onchange="checkStokAndHitung(' + itemIndex + ')">';
    itemHtml += '</div><div><label style="display:block;color:#666;margin-bottom:4px;">Harga</label>';
    itemHtml += '<div style="padding:4px;background:#f5f5f5;border:1px solid #ddd;border-radius:3px;font-size:12px;">Rp ' + parseInt(harga).toLocaleString('id-ID') + '</div></div></div>';
    itemHtml += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:12px;margin-top:8px;"><div>';
    itemHtml += '<label style="display:block;color:#666;margin-bottom:4px;">Satuan</label>';
    itemHtml += '<div style="padding:4px;background:#f5f5f5;border:1px solid #ddd;border-radius:3px;font-size:12px;">' + (satuan || '-') + '</div></div>';
    itemHtml += '<div><label style="display:block;color:#666;margin-bottom:4px;">Diskon (Rp)</label>';
    itemHtml += '<input type="number" name="items[' + itemIndex + '][diskon]" value="' + (detail.diskon || 0) + '" min="0" style="width:100%;padding:4px;border:1px solid #ddd;border-radius:3px;font-size:12px;" onchange="hitungTotal()">';
    itemHtml += '</div></div>';
    itemHtml += '<input type="hidden" name="items[' + itemIndex + '][id_barang]" value="' + detail.id_barang + '">';
    itemHtml += '<input type="hidden" name="items[' + itemIndex + '][nama_barang]" value="' + (detail.nama_barang || barangMatch.nama_barang || '') + '">';
    itemHtml += '<input type="hidden" name="items[' + itemIndex + '][kode_barang]" value="' + (detail.kode_barang || barangMatch.kode_barang || '') + '">';
    itemHtml += '<input type="hidden" name="items[' + itemIndex + '][satuan]" value="' + (satuan || '') + '">';
    itemHtml += '<input type="hidden" name="items[' + itemIndex + '][harga_satuan]" value="' + harga + '">';
    itemHtml += '<button type="button" style="width:100%;margin-top:6px;background:#ff6b6b;color:white;border:none;padding:4px;border-radius:3px;cursor:pointer;font-size:11px;" onclick="removeItem(' + itemIndex + ')">Hapus</button></div>';
...
function hitungTotal() {
    ...
    document.getElementById('subtotal_display').textContent = 'Rp ' + Math.floor(totalHarga + totalDiskon).toLocaleString('id-ID');
